    ...
    def process_record row, solr_doc = nil
      ...
      row.xpath("*").select { |x| !x.text.blank? }.each do |node|
        ...
        when "pbcoreIdentifier"
          # the IDs for wbz records are in <pbcoreIdentifier source="Accession #">
          if !node.values()[0].scan("Accession").empty?
            ...
            fields << ["video_s", "wbz/videos/#{node.text.strip}.mp4"]
            fields << ["image_s", "wbz/images/#{node.text.strip}_thumbnail.jpg"]
      ...
      get_wbz_solr_doc fields, solr_doc
  
    end
  
  
    def get_wbz_solr_doc (fields, solr_doc)
      format = wbz_title = false
      
      fields.each do |key, value|
        format ||= key == 'format'
        description ||= key == 'description_s'
        wbz_title ||= key == 'title_s'
      ...
      solr_doc [:title_s] = "wbz" unless wbz_title
